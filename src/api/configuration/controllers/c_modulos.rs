...
use crate::{errors::{AppError,AppErrorType::NotFoundError}, api::configuration::ModulesByUser}; 
...
pub async fn c_modules_by_user(
    ...
    data: Value
) -> Result<Vec<ModulesByUser>,AppError> {
    ...
    await.map(|__vec| -> Vec<ModulesByUser>{
        let mut _list_user:Vec<ModulesByUser>=Vec::new();
